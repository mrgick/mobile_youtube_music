...
from kivymd.uix.list import MDList, OneLineIconListItem, IconLeftWidgetWithoutTouch
...
class PlaylistDetailScreen(MDScreen):
    ...
    def refresh_list(self):
        ...
        for song in (x for x in self.path.iterdir() if x.is_file() and x.suffix == ".mp3"):
            item = OneLineIconListItem(text=song.stem, on_release=self.delete_song_dialog)
            item.path = song
    ...
    def delete_playlist_dialog(self):
        self.confirm_dialog(
            f'Delete playlist "{self.path.stem}"?',
